...
if __name__ == '__main__':
    ...
    writer = SummaryWriter(f'runs/{date_time_str}')
    ...
    TrainDataset = CocoDetectionDataset(TrainDir, TrainAnnotations, transform=TrainAugmentation,min_w=min_w,min_h=min_h,Mode="Train")
    ...
    TrainDataLoader = MultiEpochsDataLoader(TrainDataset,percentage=per, batch_size=batch_size,shuffle=True, num_workers=6)
    #PreTrainDataLoader = MultiEpochsDataLoader(PreTrainDataset,percentage=per, batch_size=batch_size,shuffle=True, num_workers=6)
    TestDataLoader  = MultiEpochsDataLoader(TestDataset,percentage=per,  batch_size=batch_size,shuffle=False, num_workers=8)
    ...
    for k in range(2*StartEpoch,2*no_epochs):  # Number of epochs
        ...
        for batch, (imgs1, QueryImg, bbox1,QueryBbox) in enumerate(bar):
            ...
            if Mode == 'Train':
                ...
                writer.add_scalar("Loss/train", loss, epoch * len(TrainDataLoader) + batch)
                writer.add_scalar("IoU/train", IoU, epoch * len(TrainDataLoader) + batch)
                writer.add_scalar("bbox_loss/train", bbox_loss, epoch * len(TrainDataLoader) + batch)
                writer.add_scalar("pos_class_loss/train", pos_class_loss, epoch * len(TrainDataLoader) + batch)
                writer.add_scalar("neg_class_loss/train", neg_class_loss, epoch * len(TrainDataLoader) + batch)
        ...
        if Mode == 'Test':
            ...
            # Log metrics to TensorBoard
            writer.add_scalar("Loss/test", loss, epoch * len(TrainDataLoader) + batch)
            writer.add_scalar("IoU/test", IoU, epoch * len(TrainDataLoader) + batch)
            writer.add_scalar("bbox_loss/train", bbox_loss, epoch * len(TrainDataLoader) + batch)
            writer.add_scalar("pos_class_loss/train", pos_class_loss, epoch * len(TrainDataLoader) + batch)
            writer.add_scalar("neg_class_loss/train", neg_class_loss, epoch * len(TrainDataLoader) + batch)
